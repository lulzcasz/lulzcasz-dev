...
def process_video(self, relative_path, kind):
    ...
    with download_to_temp(relative_path) as input_path:
        if kind == 'content_video':
            vf_scale_crop = "fps=24,scale='min(960,iw)':'min(620,ih)':force_original_aspect_ratio=decrease,pad=ceil(iw/2)*2:ceil(ih/2)*2"

            video_versions = ['webm', 'mp4']

            for ext in video_versions:
